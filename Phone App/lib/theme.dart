...
///
/// Kept deliberately in step with `Phone Agent/lib/theme.dart` — the two apps
/// ship the same design system, so a token added here usually belongs there too.
abstract class GsmColors {
  ...
  static const info = Color(0xFF2F7FBF);
  static const infoTint = Color(0xFFE2EEF9);
  ...
  static const dInfo = Color(0xFF5AA0DC);
}
...
class GsmSemantic extends ThemeExtension<GsmSemantic> {
  ...
  /// Brand green as a fill; [brandActive] is the readable text/icon variant on
  /// a [brandTint] background.
  final Color brand;
  ...
  static const light = GsmSemantic(
    ...
    brand: GsmColors.green500,
    brandActive: GsmColors.green700,
  ...
  static const dark = GsmSemantic(
    ...
    brand: GsmColors.green500,
    brandActive: GsmColors.greenOnDark,
    ...
    info: GsmColors.dInfo,
    infoTint: Color(0x292F7FBF),
  );
...
/// The all-caps mono label the Web App calls `.gn-eyebrow`.
TextStyle gsmEyebrow(BuildContext context, {Color? color}) => gsmMono(
      size: 10,
      color: color ?? context.cg.textMuted,
...
ThemeData _base(Brightness brightness, GsmSemantic s) {
  ...
  return ThemeData(
    ...
    inputDecorationTheme: InputDecorationTheme(
      ...
      helperStyle: TextStyle(color: s.textMuted, fontSize: 11),
    ),
    dividerTheme: DividerThemeData(color: s.borderSubtle, thickness: 1, space: 1),
