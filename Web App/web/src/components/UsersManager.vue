...
// those limits, but the API Server is what enforces them.
const me = computed(() => auth.state.user || {});
...
const roles = computed(() => (isSuperadmin.value ? ["user", "admin", "superadmin"] : ["user", "admin"]));
...
async function save() {
  ...
  try {
    if (editing.value === "new") {
      ...
      await api.patch(`/users/${editing.value}`, body);
    }
    editing.value = null;
    await load();
  } catch (e) {
